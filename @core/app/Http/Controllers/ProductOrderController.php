...
class ProductOrderController extends Controller
{
    ...
    public function flutterwave_callback(Request $request)
    {
        ...
        if (($chargeResponsecode == "00" || $chargeResponsecode == "0")){
            ...
            //update database
            self::update_database($payment_logs->id,$transaction_id);
            //send mail
    ...
    // Set TCP timeout to 30 seconds
        ...
        if (strcmp($res, "VERIFIED") == 0 || strcasecmp($res, "VERIFIED") == 0) {
            ...
            if ($receiver_email == $paypal_business_email && $order_details->payment_status == 'pending') {
                //update database
                self::update_database($order_details->id,$_POST['txn_id']);
                //send mail
                self::send_mail( $order_details);
    ...
    public function stripe_ipn(Request $request)
    {
        ...
        if (!empty($stripe_token)){
            ...
            $result = Charge::create(array(
                ...
                'source' => $stripe_token,
                'description' => 'Payment From '. get_static_option('site_'.get_default_language().'_title').'. Order ID '.$order_details->id .', Payer Name: '.$order_details->billing_name.', Payer Email: '.$order_details->billing_email,
            ));
        }
        ...
        if ($result->status == 'succeeded'){
            ...
            //update database
            self::update_database($order_details->id,$result->balance_transaction);
            //send mail
            self::send_mail( $order_details);
